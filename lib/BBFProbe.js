...
(function(root) {
  ...
  var Monitor = require('monitor-min'),
      ...
      Probe = Monitor.Probe,
      _ = Monitor._,
      logger = Monitor.getLogger('BBFProbe'),
  ...
  var BBFProbe = Probe.extend({
    ...
    */
    newTray: function(trayNum, trayTypeNum) {
      ...
      // Log
      logger.info('newTray', {trayNum:trayNum, trayTypeNum:trayTypeNum});
    },

    /**
    * Replace a button tray (probe control)
    *
    * @param options {object} Options
    *     @param options.trayNum {Number} The tray number to replace
    */
    newTray_control: function(trayNum, callback) {
      var t = this;
      trayNum = trayNum ? trayNum : 0;
      t.newTray(trayNum);
      callback(null);
    },
